...
The five metrics below are the **default monitoring set**. For experiments requiring a different primary metric, use the **Full Metrics Catalog** below.

| Full Name | Formula | Description |
|---|---|---|
| **Checkout Entry Rate** ★ Funnel KPI | `SAFE_DIVIDE(SUM(CV_views), SUM(UV))` | Percentage of visitors who viewed the checkout page — primary metric for CTA and deal page funnel experiments |
...
All available metrics from `experiments_jupiter_hist`. For full interpretation guidance see the evaluation skill. Use `SAFE_DIVIDE` for all ratio metrics.
...
| Unique Deal Viewers | `SUM(UDV)` | Visitors who reached a deal detail page — if ↓, discovery layer is the problem |
...
| Distinct bCookies | `SUM(distinct_bcookie_count)` | Secondary SRM check alongside UV — use to catch bucketing anomalies |

### Funnel Progression

| Metric | Formula | What it tells you |
|---|---|---|
| **Checkout Entry Rate** | `SAFE_DIVIDE(SUM(CV_views), SUM(UV))` | ★ All-visitor checkout funnel rate. ↓ = intent suppression before checkout. Primary guardrail for all experiments. |
| **Checkout per Deal Viewer** *(new)* | `SAFE_DIVIDE(SUM(CV_views), SUM(UDV))` | ★ PDP-to-checkout rate. If Deal View Rate is flat but this ↓, the problem is specifically on the deal detail page. |
...
| Checkout Entry Rate (unique) | `SAFE_DIVIDE(SUM(CV_viewers), SUM(UV))` | ⚠️ Verify `SUM(CV_viewers) > 0` before use — often unpopulated |
| Logged-in Checkout Entry Rate | `SAFE_DIVIDE(SUM(cv_logged_in), SUM(uv_logged_in))` | Funnel rate for logged-in users |
| Logged-out Checkout Entry Rate | `SAFE_DIVIDE(SUM(cv_logged_out), SUM(uv_logged_out))` | Funnel rate for logged-out users |
...
| **Conversion Rate** | `SAFE_DIVIDE(SUM(ue_orders), SUM(UV))` | Overall purchase rate. Blunt on its own — use Checkout-to-Order Rate to isolate checkout-specific issues. |
| **Refund Rate** | `SAFE_DIVIDE(SUM(ue_orders_refunds), SUM(ue_orders))` | ★ Tier-1 quality guardrail. ↑ = variant generates lower-quality orders (buy + regret). |
| **Cancellation Rate** | `SAFE_DIVIDE(SUM(orders_cancelled), SUM(ue_orders))` | Post-purchase intent quality. More relevant for bookable deals. |
| **Promo Order Rate** | `SAFE_DIVIDE(SUM(ue_orders_with_promo), SUM(ue_orders))` | Detects promo-driven lift. ↑ alongside ↑ CVR = check whether lift is real or bought. |
...
| **Revenue per Visitor** | `SAFE_DIVIDE(SUM(gross_revenue), SUM(UV))` | Guardrail companion to MPV. Divergence from MPV reveals margin economics shift. |
| **Margin per Order** | `SAFE_DIVIDE(SUM(margin_1_vfm), SUM(ue_orders))` | Transaction profitability. If MPV ↑ but this flat → volume drove the gain, not quality. |
| **VFM per Order** *(new)* | `SAFE_DIVIDE(SUM(vfm), SUM(ue_orders))` | Merchant-side economics per transaction. Decomposes margin into M1 vs. VFM components. |
| **Promo Spend per Order** | `SAFE_DIVIDE(SUM(promo_spend), SUM(ue_orders))` | Cost of promotional activity. ↑ = lift may be margin-dilutive. |
...
| Total M1+VFM | `SUM(margin_1_vfm)` | Use for impact estimates only — not for winner selection |
...
| *(no A/A test)* | `app` | `iphone`, `android`, `ipad` — fall back to within-experiment σ; note: *"⚠️ No A/A baseline for app platform."* |
